...
// 150812: Added a NULL check on file names in readFile, makes Visual Studio
// happier. 160302: Uses fopen_s on Windows, as suggested by Jesper Post. Should
// reduce warnings a bit. 200405: Added a few typecasts that were missing.
...
// It seems Windows/VS doesn't like fopen any more, but fopen_s is not on the
// others.
#if defined(_WIN32)
  fopen_s(&fptr, file, "r");
#else
  ...
  memset(buf, 0,
         sizeof(char) *
             (length +
              1)); /* Clean the buffer - suggested for safety by Mateusz 2016 */
...
GLuint compileShaders(const char *vs, const char *fs, const char *gs,
                      const char *tcs, const char *tes, const char *vfn,
                      const char *ffn, const char *gfn, const char *tcfn,
                      const char *tefn) {
...
// (This functionality is built-in in MicroGlut, as "glutKeyIsDown" where this
...
FBOstruct *initFBO(int width, int height, int int_method) {
  ...
  glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT24, fbo->width,
                        fbo->height);
  glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
                            GL_RENDERBUFFER, fbo->rb);
...
FBOstruct *initFBO2(int width, int height, int int_method,
  ...
  if (create_depthimage != 0) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT16, width, height, 0,
                 GL_DEPTH_COMPONENT, GL_UNSIGNED_BYTE, 0L);
...
void useFBO(FBOstruct *out, FBOstruct *in1, FBOstruct *in2) {
  ...
  if (curfbo == 0) {
    ...
    if ((w > 0) && (h > 0) && (w < 65536) &&
        (h < 65536)) // I don't believe in 64k pixel wide frame buffers for
                     // quite some time
    {
