...
// Brick pattern
void maketexture() {
  const double noiseWeight = (brickWidth * brickHeight) / (kTextureSize * kTextureSize);
  ...
  for (int x = 0; x < kTextureSize; x++) {
    for (int y = 0; y < kTextureSize; y++) {
      ...
      if (brickInX && fracYBrick < (brickHeight / (brickHeight + mortarThickness))) {
        ptex[y][x][0] = 255.0 - (fabsf(noise3(y * 0.9, x * 0.1, x * y)) * (120.0 - noiseWeight));
        ptex[y][x][1] = 100.0;
        ptex[y][x][2] = 50.0;
      } else {
        ptex[y][x][0] = 80.0 - (fabsf(snoise3(y * 0.3, x * 0.2, x * y * 0.8)) * (40.0 - noiseWeight));
        ptex[y][x][1] = 80.0 - (fabsf(snoise3(y * 0.4, x * 0.5, x * y * 0.3)) * (30.0 - noiseWeight));
        ptex[y][x][2] = 80.0 - (fabsf(snoise3(y * 0.4, x * 0.5, x * y * 0.02)) * (10.0 - noiseWeight));
      }
    }
  }
}
...
void init(void) {
  ...
  glUniform1i(glGetUniformLocation(program, "displayGPUversion"),
              1); // shader generation off
  glUniform1f(glGetUniformLocation(program, "brickWidth"), (float)brickWidth);
  glUniform1f(glGetUniformLocation(program, "brickHeight"), (float)brickHeight);
  glUniform1f(glGetUniformLocation(program, "mortarThickness"), (float)mortarThickness);
